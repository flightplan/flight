...
describe('DirtyFlags enum', () =>
{
    it('should have correct individual values', () =>
    {
        ...
        expect(DirtyFlags.Clip).toBe(1 << 2);        // 4
        expect(DirtyFlags.Cache).toBe(1 << 3);       // 8
        expect(DirtyFlags.Children).toBe(1 << 4);    // 16
    });
    ...
    {
        const expectedRender = DirtyFlags.Transform | DirtyFlags.Appearance | DirtyFlags.Clip | DirtyFlags.Cache;
        expect(DirtyFlags.Render).toBe(expectedRender);
        expect(DirtyFlags.Render).toBe(1 | 2 | 4 | 8); // 15
    });
    ...
    it('should check if specific flag is set', () =>
    {
        ...
        expect((flags & DirtyFlags.Cache) !== 0).toBe(true);
    ...
    it('should correctly unset a flag', () =>
    {
        ...
        expect(flags).toBe(DirtyFlags.Transform | DirtyFlags.Appearance | DirtyFlags.Cache | DirtyFlags.Children); // 27
    });
});
